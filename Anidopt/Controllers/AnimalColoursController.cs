...
public class AnimalColoursController : Controller {
    ...
    public async Task<IActionResult> Details(int? id) {
        if (id == null || _context.AnimalColour == null)             return NotFound();

        var animalColour = await _context.AnimalColour
            .FirstOrDefaultAsync(m => m.Id == id);
        if (animalColour == null)             return NotFound();
    ...
    public async Task<IActionResult> Edit(int id, [Bind("Colour,Id")] AnimalColour animalColour) {
        if (id != animalColour.Id)             return NotFound();
        ...
        if (ModelState.IsValid) {
            try {
                ...
                if (!AnimalColourExists(animalColour.Id))                     return NotFound();
else                     throw;
            }
            return RedirectToAction(nameof(Index));
        }
    ...
    public async Task<IActionResult> DeleteConfirmed(int id) {
        if (_context.AnimalColour == null)             return Problem("Entity set 'AnidoptContext.AnimalColour'  is null.");
        var animalColour = await _context.AnimalColour.FindAsync(id);
        if (animalColour != null)             _context.AnimalColour.Remove(animalColour);
