...
public class BreedsController : Controller {
    ...
    public async Task<IActionResult> Index() {
        if (!_breedService.Initialised)
            return Problem("Entity set 'AnidoptContext.Breed'  is null.");
        return View(await _breedService.GetAll().ToListAsync());
    }

    // GET: Breeds/Details/5
    public async Task<IActionResult> Details(int? id) {
        if (id == null || !_breedService.Initialised)
            return NotFound();
    ...
    public async Task<IActionResult> Edit(int id, [Bind("Id,Name")] Breed breed) {
        ...
        if (ModelState.IsValid) {
            try {
                ...
                if (!_breedService.ExistsById(breed.Id))
                    return NotFound();
                else
                    throw;
            }
