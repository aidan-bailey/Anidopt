...
public class UserOrganisationLinksController : Controller {
    ...
    public async Task<IActionResult> Details(int? id) {
        if (id == null || _context.UserOrganisationLink == null)             return NotFound();

        var userOrganisationLink = await _context.UserOrganisationLink
            .Include(u => u.Organisation)
            .Include(u => u.User)
            .FirstOrDefaultAsync(m => m.Id == id);
        if (userOrganisationLink == null)             return NotFound();
    ...
    public async Task<IActionResult> Edit(int id, [Bind("UserId,OrganisationId,IsAdmin,Id")] UserOrganisationLink userOrganisationLink) {
        if (id != userOrganisationLink.Id)             return NotFound();
        ...
        if (ModelState.IsValid) {
            try {
                ...
                if (!UserOrganisationLinkExists(userOrganisationLink.Id))                     return NotFound();
else                     throw;
            }
            return RedirectToAction(nameof(Index));
        }
    ...
    public async Task<IActionResult> DeleteConfirmed(int id) {
        if (_context.UserOrganisationLink == null)             return Problem("Entity set 'AnidoptContext.UserOrganisationLink'  is null.");
        var userOrganisationLink = await _context.UserOrganisationLink.FindAsync(id);
        if (userOrganisationLink != null)             _context.UserOrganisationLink.Remove(userOrganisationLink);
