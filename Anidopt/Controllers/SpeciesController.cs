...
public class SpeciesController : Controller {
    ...
    public async Task<IActionResult> Index() {
        if (!_speciesService.Initialised)
            return Problem("Entity set 'AnidoptContext.Species'  is null.");
    ...
    public async Task<IActionResult> Details(int? id) {
        if (id == null || !_speciesService.Initialised)
            return NotFound();
    ...
    public async Task<IActionResult> Edit(int id, [Bind("Id,Name")] Species species) {
        ...
        if (ModelState.IsValid) {
            try {
                ...
                if (!_speciesService.ExistsById(species.Id))
                    return NotFound();
                else
                    throw;
            }
