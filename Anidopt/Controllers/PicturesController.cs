...
public class PicturesController : Controller {
    ...
    public async Task<IActionResult> Create([Bind("AnimalId,Showcase,Id,FormFile")] PictureUpload pictureUpload) {
        if (ModelState.IsValid)             if (!PictureUpload.SupportedImageTypes.Contains(pictureUpload.FormFile.ContentType))                 ModelState.AddModelError("FormFile", "File is bad type.");
else {
    ...
    public async Task<IActionResult> DeleteConfirmed(int id) {
        if (!_pictureService.Initialised)             return Problem("Entity set 'AnidoptContext.Picture'  is null.");
